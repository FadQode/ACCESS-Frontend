...
export function DashboardSidebar({
  ...
  return (
    <>
      ...
      />
      <aside
        className={`fixed bottom-0 left-0 top-0 z-40 w-[284px] max-w-[calc(100vw-32px)] rounded-r-[20px] border border-[var(--rail-border)] bg-[var(--surface-panel)] p-3 shadow-[var(--shadow-soft)] transition-all lg:sticky lg:top-5 lg:z-auto lg:h-[calc(100vh-40px)] lg:max-w-none lg:shrink-0 lg:rounded-[20px] ${
          isOpen
            ? "translate-x-0 lg:w-[236px] lg:opacity-100"
            : "-translate-x-full lg:w-0 lg:overflow-hidden lg:border-0 lg:p-0 lg:opacity-0"
        }`}
      >
        ...
        <nav aria-label={`${ROLE_TITLE[dashboardRole]} navigation`}>
          <ul className="flex flex-col gap-2 overflow-visible pb-0">
              ...
              return (
                <li className="shrink-0 lg:shrink" key={item.label}>
                  <Link
                    ...
                    className={`flex h-11 min-w-0 items-center gap-2 rounded-xl border px-3 text-xs font-medium transition ${
                      isActive
